...
class AppointmentBase(CoreModel):
    date: date = Field(..., description="Data agendada da consulta médica.", examples=["2024-02-15"])
    time: time = Field(..., description="Horário de início da consulta.", examples=["14:30"])
...
class AppointmentRescheduleApprove(CoreModel):
    new_date: date = Field(..., description="Nova data acordada para a realização da consulta.", examples=["2024-02-22"])
    new_time: time = Field(..., description="Novo horário acordado para a consulta.", examples=["15:00"])
...
class AppointmentResponse(AppointmentBase, BaseEntitySchema):
    ...
    datetime: datetime = Field(..., description="Data e hora combinadas em formato completo ISO 8601.", examples=["2024-02-15T14:30:00Z"])
    ...
    confirmed_at: Optional[datetime] = Field(None, description="Carimbo de data/hora de quando a paciente confirmou a consulta.")
    ...
    new_date: Optional[date] = Field(None, description="Nova data em caso de solicitação de remarcação.")
    new_time: Optional[time] = Field(None, description="Novo horário em caso de solicitação de remarcação.")
